...
int main(int argz, char** argv)
{
  ...
  for (int it = 0; it < NITER; ++it) {
    ...
    uint64_t t0 = __rdtsc();
    for (int i = 0; i < TST_SZ; ++i) {
      uint64_to_ascii_2chains_base100(inpv[i], &outv[i*21]);
    }
    uint64_t t1 = __rdtsc();
    dtv[it] = t1 - t0;
    for (int i = 0; i < TST_SZ*21; ++i)
    ...
    if (it==0) {
      if (memcmp(pout, pref, TST_SZ*21) != 0) {
        printf("uint64_to_ascii_2chains_base100 fail.\n");
        return print_error(pout, pref, TST_SZ);
      }
    }
    char latbuf[32]={0};
    ...
    uint64_t t2 = __rdtsc();
    for (int i = 0; i < TST_SZ; ++i) {
      uint64_to_ascii_2chains_base100(inpv[i] ^ xx, latbuf);
      uint64_t x0, x1;
      memcpy(&x0, &latbuf[0],  sizeof(uint64_t));
      memcpy(&x1, &latbuf[16], sizeof(uint64_t));
      xx = x0 & x1 & aZero;
  ...
  for (int it = 0; it < NITER; ++it) {
    ...
    uint64_t t0 = __rdtsc();
    for (int i = 0; i < TST_SZ; ++i) {
      uint64_to_ascii_3chains_base100(inpv[i], &outv[i*21]);
    }
    uint64_t t1 = __rdtsc();
    dtv[it] = t1 - t0;
    for (int i = 0; i < TST_SZ*21; ++i)
    ...
    if (it==0) {
      if (memcmp(pout, pref, TST_SZ*21) != 0) {
        printf("uint64_to_ascii_3chains_base100 fail.\n");
        return print_error(pout, pref, TST_SZ);
      }
    }
    char latbuf[32]={0};
    ...
    uint64_t t2 = __rdtsc();
    for (int i = 0; i < TST_SZ; ++i) {
      uint64_to_ascii_3chains_base100(inpv[i] ^ xx, latbuf);
      uint64_t x0, x1;
      memcpy(&x0, &latbuf[0],  sizeof(uint64_t));
      memcpy(&x1, &latbuf[16], sizeof(uint64_t));
      xx = x0 & x1 & aZero;
